...
db_host = config['db.local']['host']
db_port = config['db.local']['port']
db_user = config['db.local']['user']
db_passw = config['db.local']['passw']
db_name = config['db.local']['db']
...
for row in rows:
	...
	command = "ffmpeg -i {0} -f mp4 -vcodec h264 -acodec libfaac {1} -y"
	os.system(command.format(source_url, out_url))
	dbCursor.execute("UPDATE videos SET estado=1 WHERE id={0}".format(v_id))
	out_video_name = vid_name.rsplit('.', 1)[0]+".mp4"
	dbCursor.execute("UPDATE videos SET video_out='{1}' WHERE id={0}".format(v_id, out_video_name))
	dbConnection.commit()
	# We send the email
	try:
		receiver = row[3]
